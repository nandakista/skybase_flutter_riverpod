...
abstract class BaseRepository with CacheMixin {
  ...
  BaseRepository({required this.storageManager});
  ...
  Future<List<T>> loadCachedList<T>({
    ...
    try {
      ...
      log('$_tag page = $page');
      if (page == 1) {
        result = await getCachedList(key: cachedKey, storage: storageManager);
        if (result.isNotEmpty) {
          onLoad().then((value) => saveCachedList(
                key: cachedKey,
                list: value,
